...
const Webgl = ( props ) => {
    const { valueA, valueB, valueC, valueO, valueR, valueG, valueGSlope } = useSelector((state) => ({
    ...
    useEffect(() => {
        ...
        plane_A_top_bottom_shape.vertices.push(
            ...
            new THREE.Vector3(Math.abs(valueA - 2.5), Math.abs((valueB - 130) / 2), -2.5), // 4,
            new THREE.Vector3(2.5, Math.abs((valueA - 130) / 2), -2.5), // 5,
            new THREE.Vector3(2.5, Math.abs((valueA - 130) / 2), 0), // 6
        ...
        return () => {
            // elementRef.current.removeChlid(scene)
        }
